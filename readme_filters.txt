...
Filter name: gamma
Filter options:
Level     A floating point number in range [0.0001, 5.0]. Values below 1.0 will decrease intensity, values above 1.0
          will increase intensity. Default: 1.0
...
Filter name: hue
Filter options:
Level     A numeric value in range [-180, 180]. Range of hue is continuous in the sense that a value of 180 is equal
          to -180. Default: 0
...
14. Mirror
Description: This filter mirrors frame content either horizontally, vertically, or on both axes.
Filter name: mirror
Filter options:
...
Background      A color value. It specifies the background color for regions that are not covered by source frame
                pixels. Default: 0 (transparent)
UpdateCenter    A boolean value that indicates whether frame center position should be adjusted to match frame content.
                Default: true
...
Type            Specifies the scaling algorithm to use. Default: nearest
                Supported types are:
                Nearest     Nearest neighbor scaling. This simple and fast algorithm duplicates or skips pixels as a
                            whole depending on scaling factor.
                Bilinear    Bilinear filtering. Interpolates neighboring pixel values to produces a smooth image.
                Bicubic     Applies more advanced calculations on a wider range of neighboring pixels to produce
                            sharper results than bilinear filtering.
...
Background      An optional color value to be used for transparent region. Only used by pixel-art filter types.
...
Description: This filter allows you to split BAM frames into multiple segments. This might be useful for large
             ...
             Note: Because of technical reasons you can only output one segment per conversion. To convert the
                   remaining segments, you can either adapt filter options "SegmentX" and "SegmentY" in the
                   configuration file for each pass, or override options "SegmentX" and "SegmentY" via command line
